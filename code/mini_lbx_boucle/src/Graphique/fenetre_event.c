...
/*   Updated: 2023/06/14 15:24:32 by yatsu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int handleConfigureNotify(int x, int y, void *data_v)
{
    (void)data_v;
...
int handle_events(void *data_v)
{
    ...
    if (width != data->len_x || height != data->len_y)
    {
        printf("La fenêtre a été redimensionnée : nouvelle taille (%d, %d)\n", width, height);
...
void	add_event(t_data *data)
{
    ...
    // mlx_hook(data->win_ptr, ConfigureRequest, 0, testConfigureRequest, data);    // ConfigureRequest event
    // mlx_hook(data->win_ptr, GravityNotify, 0, testGravityNotify, data);          // GravityNotify event
    // mlx_hook(data->win_ptr, ResizeRequest, ResizeRedirectMask, testResizeRequest, data);          // ResizeRequest event
    // mlx_hook(data->win_ptr, CirculateNotify, 0, testCirculateNotify, data);      // CirculateNotify event

	

	mlx_loop_hook(data->mlx_ptr, handle_events, data);
