...
/*   Updated: 2023/09/12 11:13:35 by yatsu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_unite(t_window *w, int xf, int yf)
{
	...
	float	x;
	float	y;
	// float	coef = (float)w->len_x / (float)w->len_y;
	xmax = 10;// * coef;
	ymax = 10;// * coef;
	xmin = -10;// * coef;
	ymin = -10;// * coef;
	x = (((float)xf / (float)w->len_y) * (xmax - xmin) * w->camera->zoom + w->camera->x_decallage);// *coef;// * w->camera->zoom ;
	y = (((float)yf / (float)w->len_y) * (ymax - ymin) * w->camera->zoom + w->camera->y_decallage);// * coef;// * w->camera->zoom ;
	// printf("x = %f\n", x);
